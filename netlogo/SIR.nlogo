...
globals [
  ...
  master-output-file       ;; single CSV file for all runs
  current-run              ;; index of current run
  current-seed             ;; random seed for current run
]

turtles-own [
  unique-id                ;; Unique identifier for each agent
  state                    ;; "S" (susceptible), "IS" (infected-symptomatic), "IA" (infected-asymptomaic), or "R" (recovered)
...
to run-multiple-simulations [num-runs]
  set master-output-file "sir_master_log.csv"
  ...
  (foreach n-values num-runs [ ? -> ? + 1 ] [
    run-id ->
    set current-run run-id
    set current-seed random 1000000

    print (word "Starting run " current-run " of " num-runs " (seed " current-seed ")")

    random-seed current-seed
    setup

    while [any? turtles with [member? state ["IA" "IS"]]] [
      go
    ]

    print (word "Finished run " current-run)
  ])

  print "All simulations complete. Data saved to sir_master_log.csv"
end
...
to log-tick-data
  foreach sort turtles [
    x ->
      ...
      file-print (word current-run "," ticks "," uid "," s "," start "," source "," infected "," is-masked)
  ]
end
